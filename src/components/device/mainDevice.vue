	...
	<div id="mainDevice">
		<div class="oneMain">
				...
				<div>
					<el-form :inline="true" :model="search" size="mini">
						...
						<el-form-item label="设备名称">
							<el-input placeholder="" v-model="search.name" @change="refreshTable()" class="input-with-select"
			...
			<div class="tableContent">
				<el-table height="679" :data="tableData" tooltip-effect="light" ref="table" size="small" v-loading="loading" class="table"
				 @selection-change="changeSelect">
					...
					<el-table-column v-for="(item,key) in tableSwitchCloumn" :prop="item.prop" :key="item.prop" :resizable="false"
					 :label="item.label" :min-width="item.width" :show-overflow-tooltip="true" align="center">
						<template slot-scope="scope">
							<div :class="{active:scope.row[item.prop]==1,'alarm-high':scope.row[item.prop]==0}"></div>
						</template>
					</el-table-column>
					<el-table-column prop="createtime" key="createtime" :resizable="false" label="创建时间" :min-width="180"
					...
					<el-table-column label="操作" width="500" key="editUpdate" align="center">
						<template slot-scope="scope">
							...
							<el-button v-if="firstType=='selection'&&rootlist.update" @click="updateOne(scope.row)" type="success" :plain="true"
		...
		<el-dialog :visible.sync="editVisible" :close-on-click-modal="false" width="750px">
			...
			<el-form ref="form" :model="form" :rules="formRule" label-width="120px" size="mini" class="form two-item-form"
			 label-position="left">
			...
			<el-form ref="form" :model="form" :rules="formRule" label-width="120px" size="mini" class="form two-item-form"
			 label-position="left">
				<el-form-item label="经度" prop="lang" style="margin-top:20px">
					<el-input class="short-input" v-model="form.lang" @change="getLocationBylatlong"></el-input>
				</el-form-item>
				<el-form-item label="纬度" prop="lat" style="margin-top:20px">
					<el-input class="short-input" v-model="form.lat" @change="getLocationBylatlong"></el-input>
				</el-form-item>
		...
		<el-dialog :visible.sync="ipVisible" :close-on-click-modal="false" width="550px">
			...
			<div class="dialogTable" v-loading="sloading">
				<el-table ref="table" height="300" :data="searchDeviceList" tooltip-effect="light" size="small" v-loading="loading"
				 class="table" border>
					<el-table-column type="index" label="序号" width="50" :resizable="false" key="indexOOO" v-if="firstType=='index'"></el-table-column>
					<el-table-column v-for="(item,key) in searchDeviceCloumn" :prop="item.prop" :key="key" :resizable="false" :label="item.label"
					 :min-width="item.width" :show-overflow-tooltip="true" align="center"></el-table-column>
		...
		<el-dialog :visible.sync="ipSetVisible" :close-on-click-modal="false" width="450px">
			...
			<el-form ref="upIpform" :model="upIpform" :rules="ipRules" label-width="80px" size="mini" class="form"
			 label-position="left">
		...
		<el-dialog :visible.sync="ipSetVisible1" :close-on-click-modal="false" width="1500px" :before-close="closeDeviceDetails">
						...
						<div class="device-content">
							...
							<div class="right-part">
								<div v-for="(item,index) in rightList" :key="index" class="one-slot">
									...
									<div v-show="item.newStatus">
										<el-button size="mini" type="text" @click="updateNewstatus(item)" v-if="item.newStatus==1">确认更新</el-button>
										<el-button size="mini" type="text" @click="updateNewstatus(item)" v-if="item.newStatus==2">确认更新</el-button>
										<el-button size="mini" type="text" @click="updateNewstatus(item)" v-if="item.newStatus==3">确认更新</el-button>
										<el-button size="mini" type="text" @click="updateNewstatus(item)" v-if="item.newStatus==4">确认更新</el-button>
										<el-button size="mini" type="text" @click="updateNewstatus(item)" v-if="item.newStatus==5">确认更新</el-button>
									</div>
								</div>
							</div>
						</div>
			...
			<div class=" dialogTable" style="margin-top:100px">
				<el-table ref="table" height="600" :data="SNData" tooltip-effect="light" size="small" class="table">
					<el-table-column prop='sndtable' label="用户自定义标签" :resizable="false" min-width="120" :render-header="renderHeader"
					 align="center">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取局站信息
			getStation1(villageId, type) {
				let _this = this;
				_this.villagestationId = villageId;
				_this.getStation(villageId, type);
			},
			...
			async getCityList(province, type) {
				...
				if (res.code) {
					...
					_this.cityList = res;
					if (type && _this.form.cityId) {
						let neg = /^\d+$/;
						let cityId = _this.form.cityId;
						if (!neg.test(cityId)) {
							let city = res.find(item => {
								return cityId == item.cityName;
							})
							if (city) {
								_this.form.cityId = city.cityId;
								_this.getCountryList(_this.form.cityId, type);
							} else {
								_this.form.cityId = "";
								_this.form.countryId = "";
								_this.form.townId = "";
			...
			async getCountryList(city, type) {
				...
				if (res.code) {
					...
					if (type && _this.form.countryId) {
						let neg = /^\d+$/;
						let countryId = _this.form.countryId;
						if (!neg.test(countryId)) {
							let city = res.find(item => {
								return countryId == item.countryName;
							})
							if (city) {
								_this.form.countryId = city.countryId;
			...
			async getTownList(country, type) {
				...
				if (res.code) {
					...
					_this.townList = res;
					if (type && _this.form.townId) {
						let neg = /^\d+$/;
						let townId = _this.form.townId;
						if (!neg.test(townId)) {
							let city = res.find(item => {
								return townId == item.townName;
							})
							if (city) {
								_this.form.townId = city.townId;
								_this.getTownList(_this.form.townId, type);
							} else {
								_this.form.townId = "";
								_this.form.villageId = "";
								_this.form.stationId = "";
			...
			getLocationBylatlong() {
				...
				if (form.lat && form.lang) {
					...
					// 根据坐标得到地址描述    
					myGeo.getLocation(new BMap.Point(form.lang, form.lat), function(result) {
						if (result) {
							console.log(result);
							let address=result.addressComponents;
							let privince = _this.provinceList.find(item => {
								return item.provinceName == address.province
							})
							if (privince) {
								_this.form.provinceId = privince.provinceId;
								_this.form.cityId = address.city;
								_this.form.countryId = address.district;
								_this.form.position=address.street+address.streetNumber;
								_this.getCityList(privince.provinceId, 1);
							}
						}
					});
				}
